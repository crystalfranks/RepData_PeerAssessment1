...
The 5-minute interval with the maximum number of steps on average across all the days is the `r maximum` interval.
...
There are `r count` missing values in the dataset.  The missing values were replace with the average number of steps for the corresponding interval.  The new mean is `r newmean` which increased from the original dataset by `r meandiff`. The new median is `r newmedian` which increased by `r mediandiff`.
